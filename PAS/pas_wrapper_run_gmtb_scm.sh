...
case "${resol_km}" in
# For the 45km case, just set it to the hard-coded values used by the
# SCM_GFS_v16beta suite that comes with the SCM.  This is just to be 
# able to compare the output from using this script (with the PAS_GFS_v16beta
# suite) to results generated by calling the run_gmtb_scm.py script 
# directly with the SCM_GFS_V16beta suite.  The correct value to use
# for a 45km grid is unknown.
  "45") regex_replace="\1""4.0, 0.15, 1.0, 1.0"
        ;;
